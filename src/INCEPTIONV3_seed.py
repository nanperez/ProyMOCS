...
for i in range(ejecucion): #Inician las ejecuciones
  ...
  # Carga el conjunto de datos de la ruta data dir 
    ...
    ruta1 = f'/home/mocs/src/Red_InceptionV3_historial_{rate}_{batch_size}_{epochs}_{i+1}_{seed[i]}_MODIF.txt'
    ruta2= f'/home/mocs/src/Red_InceptionV3_resumen_{rate}_{batch_size}_{epochs}_{i+1}_{seed[i]}_MODIF.txt'
    ...
    with open(ruta1, 'w') as f:
       for fold, (train_index, val_index) in enumerate(kf.split(images_train, labels_train)):
          ...
          #
          model.save(f'/home/mocs/src/Inceptionv3_{rate}_{batch_size}_{epochs}_{i+1}_{fold+1}_{seed[i]}_MODIF.keras')
